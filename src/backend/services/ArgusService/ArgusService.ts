...
export class ArgusService {
  ...
  async createMusclesAndEquipmentForExercises() {
    ...
    const parents = await this.exerciseService.getPage({ids: parentIds, perPage: 10000});
    ...
    for (const parent of parents.items) {
      ...
      const children = await this.exerciseService.getPage({ids: variationRows.map((x) => x.id).slice(0, 1), perPage: 10000});
