...
export const getWorkoutPlanList = RouteFactory.createRoute({
  ...
  validators: {
    ...
    response: RouteFactory.validators.paginatedResponse(workoutPlanValidator).openapi({
      description: 'List of workout plans',
    }),
  },
  handler: async (ctx) => {
    const result = await ctx.services.models.workoutPlan.paginate(ctx.viewer.id, {
      ...ctx.params.query,
      perPage: 10,
    });
    return result;
