...
const baseExerciseValidator = RouteFactory.validators.describeShape(exerciseRowValidator, {
  ...
  muscles: object({
    primary: nativeEnum(Muscle).array().openapi({description: 'List of primary muscles this exercise targets'}),
    secondary: nativeEnum(Muscle).array().openapi({description: 'List of secondary muscles this exercise targets'}),
